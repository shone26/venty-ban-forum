...
import { parseImagePaths } from '../../services/imageService';
...
export const BanForm: React.FC<BanFormProps> = ({
  ...
  const handleEvidencePathsChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    setEvidencePathsInput(e.target.value);
    setFormData({
      ...formData,
      evidencePaths: parseImagePaths(e.target.value),
    });
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Submit if valid
    if (isValid) {
      onSubmit(formData as CreateBanDto);
    }
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div className="col-span-full">
            ...
            <textarea
              ...
              rows={2}
              className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
              placeholder="Enter image paths separated by commas (e.g., image1.jpg, image2.jpg)"
            />
            <p className="mt-1 text-xs text-gray-500">
              Enter relative paths to evidence images, separated by commas. These will be displayed on the ban page.
            </p>
          </div>
